...
if __name__ == '__main__':
    ...
    parser.add_argument("-n", "--nsplit", type=int, help="number of partitions", default=100)
    parser.add_argument("-o", "--output", type=str, help="dir of output", required=True)
    parser.add_argument("--normalize", type=int, help="normalized or not", default=1)
    parser.add_argument("--step", type=int, help="step of file sizes", default=8)
    ...
    sort_idx = np.argsort(y_train[:, 0], axis=0)
    ...
    base_size = batch_size - (0 + args.step * (args.nsplit-1)) / 2
    bias_size = 0
    i_start = 0
    i_end = 0
    ...
    for i in range(int(nice_n_train // batch_size)):
        ...
        i_start = int(i_end)
        i_end = int(i_start + base_size + bias_size)
